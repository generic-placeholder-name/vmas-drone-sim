...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
            ...
            for y in torch.arange(self.grid_resolution/2, world_height, self.grid_resolution):
                ...
                for reward_area in self.env_config["rewardAreas"]:
                    if is_point_in_polygon(point, reward_area): # TODO: Check that point not in penalty areas
                        ...
                        self.waypoints.append(Waypoint(torch.Tensor(convert_to_original_units(scaled_point)), goal, reward_radius=self.reward_radius))
        ...
        for (x, y) in self.agent_start_pos:
            ...
            # if agent in point
            world.add_landmark(goal)
            self.waypoints.append(Waypoint(torch.Tensor(convert_to_original_units(point)), goal, reward_radius=self.reward_radius))
